...
def search_product_list(interval_count=1, interval_hours=6):
    ...
    """
    # prod_tracker = pd.read_csv('/home/bilal/IA/amazon_webscraper-master/trackers/TRACKER_PRODUCTS.csv', sep=';')
    ...
    while interval < interval_count:
        ...
        for x, url in enumerate(prod_tracker_URLS):
            ...
            try:
                ...
                try:
                    price = float(
                        soup.find(id='priceblock_ourprice').get_text().replace(',', '.').strip().replace('CDN$', '').strip().replace('\xa0', '').strip())
                    print(price)
                except:
                    price = ''
    ...
    last_search = glob('/home/bilal/IA/amazon_webscraper-master/search_history/*.xlsx')[
